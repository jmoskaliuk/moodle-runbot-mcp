...
function buildInstanceUrl(instance: MoodleInstance): string {
...
export function registerSnapshotList(server: McpServer): void {
  server.registerTool(
    "snapshot_list",
    {
...
export function registerSnapshotCreate(server: McpServer): void {
  server.registerTool(
    "snapshot_create",
    {
...
Returns:
  { snapshotId, file, sizeFormatted, createdAt }`,
    ...
    async ({ instanceId, snapshotId, label, description, plugins }) => {
      ...
      try {
        const meta = await snapshot.createSnapshot(
          instance, snapshotId, label, description, plugins
        );
...
export function registerSnapshotDelete(server: McpServer): void {
  server.registerTool(
    "snapshot_delete",
    {
...
export function registerSnapshotBuild(server: McpServer): void {
  server.registerTool(
    "snapshot_build",
    {
...
Returns:
  { snapshotId, label, file, sizeFormatted, instanceId, durationSeconds }`,
    ...
    async ({
      prId, branch, pluginSrcPath, pluginType, pluginName,
      moodleVersion, phpVersion, db,
      snapshotId, label, description, plugins,
    }) => {
      ...
      let meta: snapshot.SnapshotMeta | undefined;
      ...
      try {
        ...
        meta = await snapshot.createSnapshot(
          instance, snapshotId, label, description, plugins
        );
      } catch (e) {
        buildError = e;
        ...
        await nginx.unregisterInstance(instance.id).catch((e: unknown) => {
          console.error(`[snapshot_build] nginx unregister ${instance.id}:`, e);
        });
        await docker.stopContainers(instance).catch((e: unknown) => {
          console.error(`[snapshot_build] docker stop ${instance.id}:`, e);
        });
        await docker.cleanupInstanceDir(instance).catch((e: unknown) => {
