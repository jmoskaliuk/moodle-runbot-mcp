...
async function runHTTP(): Promise<void> {
  ...
  function publicAgreements(list: orders.AgreementRecord[] | undefined): Array<{
    type: "agb" | "avv";
  ...
  const customerDashboardHandler: express.RequestHandler = async (req, res) => {
    const { token } = req.params;
    try {
  ...
  const customerAgreementHandler: express.RequestHandler = async (req, res) => {
    const { token, type } = req.params;
    if (type !== "agb" && type !== "avv") {
    ...
    try {
      const order = await orders.getOrderByCustomerMagicToken(token);
      if (!order) { res.status(404).json({ error: "Kunden-Link nicht gefunden oder abgelaufen" }); return; }
      ...
      res.setHeader("Content-Disposition",
        `inline; filename="${type.toUpperCase()}-${order.id}-${result.templateVersion}.pdf"`);
  ...
  app.get("/admin/orders", adminAuth, async (req, res) => {
    try {
      const stateFilter = (req.query.state as string | undefined)?.toUpperCase();
      const since       = (req.query.since as string | undefined);
      const filter: Parameters<typeof orders.listOrders>[0] = {};
      if (stateFilter) filter.state = stateFilter as orders.OrderState;
  ...
  const adminOrderRejectHandler: express.RequestHandler = async (req, res) => {
    const { id } = req.params;
  ...
  const adminOrderTerminateHandler: express.RequestHandler = async (req, res) => {
    const { id } = req.params;
    ...
    try {
      ...
      if (order.instanceId) {
        ...
        if (inst) {
          ...
          await saveInstance(inst);
          await nginxSvc.unregisterInstance(inst.id).catch((e: unknown) =>
            console.error(`[admin/orders] WARN nginx unregister:`, e));
          await dockerSvc.stopContainers(inst).catch((e: unknown) =>
            console.error(`[admin/orders] WARN docker stop:`, e));
          await dockerSvc.cleanupInstanceDir(inst).catch((e: unknown) =>
            console.error(`[admin/orders] WARN cleanup dir:`, e));
