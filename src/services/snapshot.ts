...
export async function restoreSnapshot(
  ...
  try {
    let oldWwwroot: string | null = null;
    if (instance.db === "pgsql") {
      const result = await run(
        `${env} ${bin} exec -T db psql -U moodle -t -c ` +
        `"SELECT value FROM mdl_config WHERE name='wwwroot';" moodle`
      ).catch(() => "");
      oldWwwroot = result.trim() || null;
    } else if (instance.db === "mariadb" || instance.db === "mysql") {
      const result = await run(
        `${env} ${bin} exec -T db mysql -u moodle -pm@0dl3ing -N -B -e ` +
        `"SELECT value FROM mdl_config WHERE name='wwwroot';" moodle`
      ).catch(() => "");
      oldWwwroot = result.trim() || null;
    }
    ...
    if (oldWwwroot && oldWwwroot !== newWwwroot) {
      console.error(
        `[snapshot] URL-Rewrite: "${oldWwwroot}" → "${newWwwroot}" …`
      );
      await run(
