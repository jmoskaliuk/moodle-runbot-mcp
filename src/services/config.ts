...
const DIRECTUS_CONFIG_COLLECTION =
  process.env.DIRECTUS_CONFIG_COLLECTION ?? "runbot_demo_config";
const DIRECTUS_CONFIG_TOKEN =
  process.env.DIRECTUS_CONFIG_TOKEN
...
function asBool(value: unknown): boolean | undefined {
  ...
  if (["true", "1", "yes", "ja", "on", "published", "active", "visible"].includes(normalized)) return true;
  if (["false", "0", "no", "nein", "off", "draft", "archived", "hidden"].includes(normalized)) return false;
  return undefined;
}
...
function pluginRefFromComponent(component: string, explicitType?: string, explicitName?: string): PluginRef | null {
...
function normalizeFileConfig(config: DemoConfig): DemoConfig {
  return {
    ...config,
...
async function loadFileConfigsRaw(): Promise<DemoConfig[]> {
  try {
...
function mapDirectusItemToConfig(item: DirectusItem): DemoConfig | null {
  ...
  return {
    ...
    description:
      ...
      ?? "",
    features:
      asStringArray(item.features).length > 0
...
async function fetchDirectusConfigs(): Promise<DemoConfig[]> {
  ...
  const url =
    `${DIRECTUS_URL}/items/${DIRECTUS_CONFIG_COLLECTION}` +
    `?limit=-1&fields=*.*.*`;
  const res = await fetch(url, { headers });
  if (!res.ok) {
    const text = await res.text().catch(() => "");
    throw new Error(
      `Directus-Config-Load fehlgeschlagen (${res.status} ${res.statusText}): ${text.slice(0, 300)}`,
    );
  }
  ...
  return rows
    ...
    .filter((row): row is DemoConfig => Boolean(row));
}
...
export async function loadConfigs(): Promise<DemoConfig[]> {
  ...
  if (useDirectus || useHybrid) {
    try {
      ...
      if (useDirectus) {
        throw new Error(
          `Directus lieferte 0 Einträge aus Collection '${DIRECTUS_CONFIG_COLLECTION}'`,
        );
      }
      console.error(
        `[config] Directus lieferte keine Configs — Fallback auf ${CONFIG_FILE}`,
      );
    } catch (e) {
      if (useDirectus) throw e;
