...
export async function cleanupOrphans(): Promise<OrphanCleanupReport> {
  ...
  // ── Log-Zusammenfassung ────────────────────────────────────────────────────
  const total =
    report.orphanContainers.length +
  ...
  if (total === 0 && report.errors.length === 0) {
    ...
    console.error(
      `[cleanup] Startup: removed ${report.orphanContainers.length} container(s), ` +
 ...
 */
async function findOrphanContainerIds(knownIds: Set<string>): Promise<string[]> {
  const { stdout } = await execAsync(
    `docker ps -a --format '{{.Names}}' --filter "name=runbot-"`
  );
 ...
 */
async function findOrphanDirs(knownIds: Set<string>): Promise<string[]> {
  let entries: string[];
  try {
 ...
 */
async function findOrphanNginxConfigs(knownIds: Set<string>): Promise<string[]> {
  let files: string[];
  try {
 ...
 */
async function removeOrphanContainer(id: string): Promise<void> {
  const project = `runbot-${id}`;
  // Alle Container mit diesem Compose-Projekt finden (auch gestoppte)
  const { stdout } = await execAsync(
    `docker ps -a --format '{{.Names}}' --filter "name=^${project}-"`
  );
  const names = stdout.split('\n').map((s) => s.trim()).filter(Boolean);
...
async function reloadNginxAfterCleanup(): Promise<void> {
  try {
