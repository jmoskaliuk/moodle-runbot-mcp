 ...
 */
function instancePluginSlug(instance: MoodleInstance): string {
  if (instance.configId) return instance.configId;
...
async function authMiddleware(
  ...
  (req as unknown as { runbotInstance: MoodleInstance }).runbotInstance = instance;
  next();
}
 ...
 */
export function buildInternalRouter(): express.Router {
  ...
  router.post("/snapshot/create", async (req, res) => {
    const instance = (req as unknown as { runbotInstance: MoodleInstance }).runbotInstance;
    ...
    try {
      const meta = await createSnapshot(
        ...
        snapshotId,
        `${slug} — ${label}`,
        description || `Created from in-Moodle admin by instance ${instance.id}`,
        [slug]
      );
      res.json({
        snapshotId: meta.id,
  ...
  router.post("/snapshot/delete", async (req, res) => {
    ...
    try {
      ...
      if (config?.snapshotId === id) {
        res.status(409).json({
          error:
            `Snapshot '${id}' ist aktuell als Default gesetzt. ` +
