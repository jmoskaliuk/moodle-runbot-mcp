...
import { confirmEmail, readyEmail, errorEmail, mailVerifyOrder, mailOrderReview, mailOrderConfirmed, mailAdminAlertNewOrder, } from "./emailTemplates.js";
// ── SMTP-Konfiguration ────────────────────────────────────────────────────────
...
function firstNameOf(request) {
    return (request.name || "").split(" ")[0] || "";
}
// ── E-Mail: Bestätigung ───────────────────────────────────────────────────────
export async function sendConfirmationEmail(request, pluginName) {
...
export async function sendDemoReadyEmail(request, pluginName, demoUrl) {
...
function orderFirstName(order) {
    return (order.signer?.name || "").split(" ")[0] || "";
}
/** (1) Double-Opt-In nach Formular-Submit. */
export async function sendVerifyOrderEmail(order, configName) {
...
export async function sendOrderReviewEmail(order, configName) {
...
export async function sendOrderConfirmedEmail(order, configName, opts) {
...
export async function sendAdminAlertNewOrderEmail(order, configName) {
