 ...
 */
function renderTemplate(markdown, vars) {
    return markdown.replace(/\{\{([a-zA-Z_][a-zA-Z0-9_]*)\}\}/g, (_match, key) => {
 ...
 */
function buildTemplateVars(order, config, opts) {
 ...
 */
function runPandoc(args, stdinContent) {
 ...
 */
async function renderPdf(kind, order, config, opts) {
    ...
    if (code !== 0) {
        throw new Error(`pandoc (${kind}) beendet mit Exit ${code}. stderr=${stderr.slice(-800)}`);
    }
 ...
 */
export async function findExistingPdf(orderId, kind) {
 ...
 */
export async function ensureContractPdf(order, config, kind, opts) {
