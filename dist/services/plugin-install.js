...
function parseGithubUrl(gitUrl) {
    ...
    if (!m) {
        throw new Error(`Ungültige GitHub-URL. Format: https://github.com/owner/repo (keine SSH, keine lokalen Pfade).`);
    }
    const owner = m[1];
    const repo = m[2];
 ...
 */
async function parseVersionPhp(filePath) {
 ...
 */
export async function clonePluginFromGithub(gitUrl) {
 ...
 */
export async function listAllConfigsRaw() {
