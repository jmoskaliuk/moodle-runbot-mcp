...
export async function restoreSnapshot(instance, snapshotFile) {
    ...
    const debugSqlPg = `UPDATE mdl_config SET value='0' WHERE name IN ('debug','debugdisplay','debugstringids','debugsmtp','debugpageinfo','debugvalidators','perfdebug','debugusers','debugsqltrace');`;
    const debugSqlMy = debugSqlPg; // Syntax identisch
    if (instance.db === "pgsql") {
        await run(`${env} ${bin} exec -T db ` +
            `psql -U moodle -c "${debugSqlPg}" moodle`).catch((e) => {
            console.error(`[snapshot] WARN: debug-reset failed (pgsql, task36): ${String(e)}`);
        });
    }
    else if (instance.db === "mariadb" || instance.db === "mysql") {
        await run(`${env} ${bin} exec -T db ` +
            `mysql -u moodle -pm@0dl3ing -e "${debugSqlMy}" moodle`).catch((e) => {
