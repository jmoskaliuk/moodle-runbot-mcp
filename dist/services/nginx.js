 ...
 */
export async function registerInstance(instanceId, port) {
    if (!NGINX_ENABLED)
        return; // Lokal ohne Domain: kein nginx nötig
    ...
    if (!certOk) {
        console.error(`[nginx] WARNING: Wildcard-Cert fehlt unter ${SSL_CERT_DIR}/ — ` +
 ...
 */
export async function cleanupAllConfigs() {
    if (!NGINX_ENABLED)
        return;
    try {
        const files = await fs.readdir(NGINX_CONF_DIR);
        const stale = files.filter(f => (f.startsWith("runbot-") || f.startsWith("demo-")) && f.endsWith(".conf"));
